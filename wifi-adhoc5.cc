...
private:
   ...
   * \param node The node.
   */
  void AdvancePosition(Ptr<Node> node, NodeContainer& nodes); // Add a parameter for NodeContainer
  /**
...
void Experiment::AdvancePosition(Ptr<Node> node, NodeContainer& nodes) // Add the parameter here
{
  ...
  // Schedule the AdvancePosition method for the next node
  uint32_t nodeId = node->GetId();
  if (nodeId < 4)
  {
    Simulator::Schedule(Seconds(1.0), &Experiment::AdvancePosition, this, nodes.Get(nodeId + 1), nodes);
  }
}
                                 ...
                                 const WifiMacHelper &wifiMac, const YansWifiChannelHelper &wifiChannel)
{
  ...
  NodeContainer c;
  c.Create(5); // Create NodeContainer here
  ...
  positionAlloc->Add(Vector(0.0, 5.0, 0.0));  // Add positions for 3 more nodes
  ...
  // Install applications for the remaining nodes (nodes 2 to 4)
  ...
  Simulator::Schedule(Seconds(1.5), &Experiment::AdvancePosition, this, c.Get(1), c); // Pass NodeContainer as a parameter
...
int main(int argc, char *argv[])
{
  ...
  {
    experiment = Experiment("Node" + std::to_string(i + 1) + "-54mb");
